...
export async function syncOutOfStock(options: StockSyncOptions) {
    ...
    const context = await getInventoryContext(shop, shopifyProductId);
    if (!context) return;

    const { client, inventoryItemId, locationId } = context;
...
export async function syncInStock(options: StockSyncOptions) {
    ...
    // Restore to a default high number or some logic to mark as available
    await client.request(mutation, {
        variables: {
            input: {
                reason: "restock",
