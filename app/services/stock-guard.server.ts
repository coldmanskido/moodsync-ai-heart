...
export async function syncOutOfStock({ shop, shopifyProductId }: { shop: string, shopifyProductId: string }) {
...
export async function syncInStock({ shop, shopifyProductId }: { shop: string, shopifyProductId: string }) {
    ...
    // 3. Set Inventory to a default value (e.g. 100)
    const mutation = `mutation inventorySetHandQuantities($input: InventorySetHandQuantitiesInput!) {
        inventorySetHandQuantities(input: $input) {
            userErrors {
                field
